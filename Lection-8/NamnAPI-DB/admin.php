...
<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($lastname)) {
      $stmt = $conn->prepare("INSERT INTO lastNames (lastNames) VALUES (:lastNames)");
    ...
    if ($gender === 'male') {
      ...
      $stmt = $conn->prepare("INSERT INTO firstNamesMale (firstNamesMale) VALUES (:firstNamesMale)");
      $stmt->bindParam(':firstNamesMale', $firstNameMale);
      $stmt->execute();
    } else {
      $firstNameFemale = htmlspecialchars($_POST['firstname']);
      $stmt = $conn->prepare("INSERT INTO firstNamesFemale (firstNamesFemale) VALUES (:firstNamesFemale)");
